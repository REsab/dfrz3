...
public class StudentCtrl {
    ...
    public void insert() {
        ...
        if (studentService.exists(student.id) == 1) {
    ...
    public void update() {
        ...
        if (1 == studentService.exists(student.id)) {  //寻找是否有这个学号 的学生
            Integer index = studentService.searchIdIndex(student.id);  //有这个学生再找他的下标位置
            System.out.println("请输入 " + student.id + "号的新名字");
            student.name = scanner.next();
            System.out.println("请输入 " + student.id + "号的性别");
            student.sex = scanner.next();
    ...
    public void delete() {
        ...
        student.id=scanner.nextInt();
             ...
             int result=studentService.delete(student.id);
